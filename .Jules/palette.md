...
## 2026-06-30 - Keyboard Accessibility for Custom Div Cards
**Learning:** Custom div-based components used as cards in this app (`.grade-card`, `.subject-card`) lack native keyboard focus and interaction by default, causing screen reader and keyboard navigation issues.
**Action:** Always ensure that interactive non-semantic elements are accompanied by `role="button"`, `tabindex="0"`, and event listeners for both `click` and `keydown` ('Enter' and 'Space'), and call `e.preventDefault()` for the 'Space' key to prevent unwanted page scrolling.
