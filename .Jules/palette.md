...
## 2024-06-24 - Interactive DIVs keyboard accessibility
**Learning:** In this application, `.grade-card` and `.subject-card` act as interactive buttons but were implemented as generic `<div>` tags lacking semantic meaning and native keyboard support, which made them inaccessible to users relying on keyboard navigation (Tab/Enter/Space).
**Action:** When adding custom interactive elements using non-semantic tags (like `div` or `span`) in this project, always explicitly provide `role="button"`, `tabindex="0"`, appropriate `aria-label`s, and `keydown` event listeners for 'Enter' and 'Space' keys (including `e.preventDefault()` for the Space key to prevent unwanted page scrolling) to ensure full keyboard accessibility.
