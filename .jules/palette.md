...
## 2024-11-20 - Adding semantic IDs and ARIA labels for Screen Reader compatibility
**Learning:** For custom modals that don't use `<dialog>`, they do not get native accessibility functions such as focus trapping, semantic definitions (e.g. `role=dialog`), and appropriate associations for their descriptions and titles via `aria-describedby` and `aria-labelledby`.
**Action:** Always verify that `<div class="modal">` patterns have `role="dialog"`, `aria-modal="true"`, and the appropriate elements correctly map with their IDs to `aria-labelledby` and `aria-describedby` for compatibility with screen readers.
